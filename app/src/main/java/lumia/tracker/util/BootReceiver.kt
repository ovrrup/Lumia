            ...
            if (action == Intent.ACTION_BOOT_COMPLETED || 
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        ...
                        Log.e("BootReceiver", "Error restoring notifications on boot/update", e)
                    } finally {
                        try {
                            pendingResult.finish()
                        } catch (e: Exception) {
                            Log.e("BootReceiver", "Error finishing pending result", e)
                        }
                    }
                }
            }
        } catch (e: Exception) {
            Log.e("BootReceiver", "Top level error in BootReceiver onReceive", e)
        }
    }
}
