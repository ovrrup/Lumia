...
/**
 * Handles end-to-end encryption (AES-256-GCM) and mutual challenge-response authentication
 * (HMAC-SHA256) for zero-trust multi-device peer-to-peer data syncing.
 */
object SyncCryptoManager {
    ...
    /**
     * Generates a 6-digit numeric PIN for user pairing verification.
     */
    ...
    /**
     * Derives a 256-bit symmetric AES key from the pairing PIN and session nonce.
     */
    private fun deriveKey(pin: String, nonce: String): SecretKeySpec {
     ...
     */
    fun calculateAuthHash(pin: String, nonce: String, deviceId: String): String {
        val mac = Mac.getInstance("HmacSHA256")
        val keySpec = SecretKeySpec(pin.toByteArray(Charsets.UTF_8), "HmacSHA256")
        mac.init(keySpec)
     ...
     */
    fun verifyAuthHash(pin: String, nonce: String, deviceId: String, receivedHash: String): Boolean {
        return try {
            val expected = calculateAuthHash(pin, nonce, deviceId)
     ...
     */
    fun encryptPayload(plainBytes: ByteArray, pin: String, nonce: String): Pair<String, String> {
        val keySpec = deriveKey(pin, nonce)
     ...
     */
    fun decryptPayload(encryptedBase64: String, ivBase64: String, pin: String, nonce: String): ByteArray {
        val keySpec = deriveKey(pin, nonce)
